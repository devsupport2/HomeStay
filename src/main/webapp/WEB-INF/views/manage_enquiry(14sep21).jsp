										...
										<table class="table no-margin">
											...
											<tbody>
												...
												<tr ng-repeat="item in getEnquiries" style="cursor:pointer; cursor:hand;" ng-style="(item.status == 'w' || item.status == 'cw') && {'color':'green'} || (item.status == 'l' || item.status == 'cl') && {'color':'red'} || item.status == 'o' && {'color':'blue'} || {'color': '#202020'}">
													...
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.firstName}} {{item.lastName}}<br>{{item.userCompanyName}} ,{{item.state}},{{item.city}}<i class="fa fa-info-circle" data-toggle="tooltip" title="" data-original-title="Email: {{item.email}}, Mobile: {{item.mobileNumber}}" aria-hidden="true"></i></td>
