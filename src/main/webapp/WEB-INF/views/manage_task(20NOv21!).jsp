...
<!-- 									      <h6 class="box-title">Today's Follow-up ({{currentdate | date:'dd-MM-yyyy'}})</h6> -->
								...
								<div class="box-body">
									<div class="table-responsive">
										<table class="table no-margin">											
											<tbody>												
												<tr ng-repeat="item in followupEnquiriesByDate1" style="cursor:pointer; cursor:hand; border-left: 8px solid #eee; border-radius: 3px;" ng-style="item.followupStatus == 'y' && {'border-color': '#DCDCDC'} || {'border-color': 'green'}">
													...
													<!-- <td width="70%" ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal">{{item.remark}}<span ng-if="item.firstName != null"><br>{{item.firstName}} {{item.lastName}} - {{item.userCompanyName}}</span></td> -->
													 <td width="70%" ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal">{{item.remark}}<span ng-if="item.firstName != null"><br></span></td> 
