	...
	*/	}
		...
		$scope.addClientCategory = function() {		
			if(!$scope.categoryname) {			
				$scope.errorCategory = 1;
				$scope.msgCategory = "Please enter category";
				document.getElementById("categoryname").focus();
			} else if(!$scope.categorycode)
			{
				$scope.errorName = 1;
				$scope.msgCategory = "Please enter Category code";
				document.getElementById("categorycode").focus();
			}
			else{
				//$scope.userTypeSpin = 1;	
				$scope.clientCategorySpin = 1;	
				var link = baseUrl+'addClientCategory?categoryname='+$scope.categoryname+'&categorycode='+$scope.categorycode;
				$http.post(link).success(function(data, status, headers, config) {
					$scope.addclientcategory = data;
					/*
					if(addclientcategory="Success")
					{
						$scope.clientCategorySpin = 0;	
						$scope.clientCategorySuccess = 1;
						$scope.clientCategorySuccessMsg = "Data added successfully.";
					}else
						{
						$scope.clientCategorySpin = 0;	
						$scope.clientCategoryError = 1;
						$scope.clientCategoryErrorMsg = "something wrong ";
					...
					$scope.addclientcategory = "Response Fail";
				});
			}
		}
