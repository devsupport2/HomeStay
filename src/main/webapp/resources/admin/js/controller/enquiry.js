app.controller('enquiryCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' ,
	...
	/*$scope.getReferenceById = function(RefId) {
		alert("hhdhdh");
		
		
			var link = baseUrl+'getRefenceByRefId?RefId='+RefId;
			$http.get(link).success(function(data, status, headers, config) {
				$scope.getRefDataById = data;
			}).error(function(data, status, headers, config) {
				$scope.getRefDataById = "Response Fail";
			});
	...
	/*$scope.searchEnquiry = function() {
		alert("krishdrdtna");
	$scope.startDate = document.getElementById("start-date").value;
	$scope.endDate = document.getElementById("end-date").value;
	...
	var date = new Date($scope.startDate); // Or your date here
	$scope.StartDate= ((date.getMonth() + 1).toString().padStart(2, '0') + '/' + date.getDate().toString().padStart(2, '0') + '/' +  date.getFullYear());
	alert($scope.StartDate);
	
	var date1 = new Date($scope.endDate); // Or your date here
	$scope.EndDate=((date1.getMonth() + 1) + '/' + date1.getDate().toString().padStart(2, '0') + '/' +  date1.getFullYear());
	alert($scope.EndDate);
	
	
	
	var link = baseUrl+'getEnquiriesByPage?StartDate='+$scope.StartDate+'&EndDate='+$scope.EndDate+'&pagesize='+$scope.pageSize+'&startindex='+$scope.startindex;
	$http.get(link).success(function(data, status, headers, config) {
		$scope.getEnquiries = data;
	}).error(function(data, status, headers, config) {
		$scope.getEnquiries = "Response Fail";
	...
	$scope.searchEnquiry = function() {
		...
		if(search == undefined || search == "") {
			...
			//	alert($scope.endDate);
				...
				var date1 = new Date($scope.endDate); // Or your date here
				$scope.EndDate=((date1.getMonth() + 1).toString().padStart(2, '0') + '/' + date1.getDate().toString().padStart(2, '0') + '/' +  date1.getFullYear());
				//alert($scope.EndDate);
				
				var link = baseUrl+'getEnquiriesByPage?StartDate='+$scope.StartDate+'&EndDate='+$scope.EndDate+'&pagesize='+$scope.currentPage+'&startindex='+$scope.startindex;
				//alert("123",link);
				console.log("379",link);
				$http.get(link).success(function(data, status, headers, config) {
					$scope.getEnquiries = data;
					console.log("379",$scope.getEnquiries);
				}).error(function(data, status, headers, config) {
					$scope.getEnquiries = "Response Fail";
				});
...
/*	$scope.searchEnquiry = function() {
	...
	*/
	...
	$scope.addEnquiry = function() {
		...
		if($scope.enquirydate==undefined || $scope.enquirydate=="") {
		...
		} else if($scope.usertypeidadd==undefined || $scope.usertypeidadd=="") {
			document.getElementById("usertypeidadd").focus();
			$scope.info = 1;
			$scope.message = "Please select enquiry ReferenceType!";
			$timeout(function(){
				$scope.info = 0;
			}, 2000);
			...
			//alert($scope.enquiryviaadd);	
			//alert($scope.usertypeidadd);	
			//alert($scope.clientidadd);		
			var link = baseUrl+'addEnquiry?enquirydate='+$scope.enquirydate+'&enquiryvia='+$scope.enquiryviaadd+'&referenceid='+$scope.usertypeidadd+'&clientid='+$scope.clientidadd+'&employeeid='+$scope.employeeidadd+'&remark='+$scope.remarksadd;			
	...
	*/
	...
	//category
	var link = baseUrl+'getAllCategory';
	$http.get(link).success(function(data, status, headers, config) {
	...
	$scope.addUser2 = function() {						
		...
		if($scope.usertypenameadd != 2 && !$scope.hourlywagesadd) {
			$scope.hourlywagesadd = 0;
		}
		if($scope.usertypenameadd != 2 && !$scope.overtimewagesadd) {
			$scope.overtimewagesadd = 0;
		}
		...
		if(!$scope.usertypenameadd) {				
			...
			var formData=new FormData();
			formData.append("profilepicture",profilepictureadd1.files[0]);	
	...
	$scope.getUser = function(clientid) {

		var userid=$scope.clientid;
	...
	/*	else if($scope.usertypename == 2 && !$scope.hourlywagesedit) {				
			...
			$http({method: 'POST',
		    	...
		    	if($scope.edituser == 'Success'){	
		    		...
		    		var link = baseUrl+'getUsersByPage?pagesize=20&startindex='+$scope.startindex;
					$http.get(link).success(function(data, status, headers, config) {
						$scope.getUsers = data;
	...
	$scope.addTaskList = function(){
		 ...
		 var d=0;
		//$scope.taskdate = document.getElementById("datepicker").value;	
		 $scope.taskDate = document.getElementById("datepicker").value;		
		$scope.employeeidadd = 0;
		
			var link = baseUrl+'addTask';
			$http({url: link, method: "POST", data: $scope.Task}).success(function(data, status, headers, config) {
				$scope.addTaskData = data;
				console.log($scope.addTaskData);
				//toastr.success('Data Saved!');
				//$scope.getAllCityList();
				
				angular.forEach($scope.assigntasklist,function(item) {	
				console.log(link);
					var link = baseUrl+'addTaskAssign?userid='+item.userId;
					$http.post(link).success(function(data, status, headers, config) {
						$scope.addTaskAssignassign = data;
						d = d + 1;
						if($scope.assigntasklist.length == d) {
							$scope.spin = 0;
							$scope.success = 1;
							$scope.message = "Task Added Successfully.";
							$timeout(function(){
								$scope.success = 0;
								$window.location.href = adminurl+'manage_task2';
							}, 2000);
						}
					}).error(function(data, status, headers, config) {
						$scope.addTaskAssignassign = "Response Fail";
					});
				});
				
				
			}).error(function(data, status, headers, config) {
				$scope.addTaskData = "Response Fail";
	...
	$scope.addAssignTaskRow = function() {
		...
		if($scope.employeeidadd==undefined || $scope.employeeidadd=="") {			
			...
			$http.get(link).success(function(data, status, headers, config) {
            	...
            	$scope.assigntasklist.push({'userId':$scope.employeeidadd, 'firstName':$scope.firstname, 'lastName':$scope.lastname});
    			console.log($scope.assigntasklist);
            	$scope.employeeidadd = "";
	...
	var link = baseUrl+'getAllTask'; 
	$http.get(link).success(function(data, status, headers, config) {
		$scope.TaskList = data;
		
	}).error(function(data, status, headers, config) {
		$scope.TaskList = "Response Fail";
	});		
	...
	$scope.searchReferenceType = function(item) {
		//alert("krishna");
		//$scope.search = '';
	...
	//alert("first");
				...
				//alert("SAD");
				$scope.startDate = document.getElementById("start-ref-date").value;
				$scope.endDate = document.getElementById("end-ref-date").value;
				
		//alert($scope.startDate);
		//alert($scope.endDate);
		//alert($scope.referenceId)	
				var date = new Date($scope.startDate); // Or your date here
				$scope.StartRefDate= (date.getDate().toString().padStart(2, '0') + '/' +(date.getMonth() + 1).toString().padStart(2, '0') + '/' +  date.getFullYear());
				//alert($scope.StartRefDate);
				
				var date1 = new Date($scope.endDate); // Or your date here
				$scope.EndRefDate=(date1.getDate().toString().padStart(2, '0') + '/'+ (date1.getMonth() + 1).toString().padStart(2, '0') + '/' +  date1.getFullYear());
				//alert($scope.EndRefDate);
				
				var link = baseUrl+'getReferencesByDateId?StartRefDate='+$scope.StartRefDate+'&EndRefDate='+$scope.EndRefDate+'&RefTypeId='+$scope.referenceId;
				//alert("123",link);
				console.log("379",link);
				$http.get(link).success(function(data, status, headers, config) {
					$scope.getRefencesReportEnquiries = data;
					console.log($scope.getRefencesReportEnquiries);
				}).error(function(data, status, headers, config) {
					$scope.getRefencesReportEnquiries = "Response Fail";
				});
