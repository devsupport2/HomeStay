app.controller('generateQuotationCtrl', [ '$scope', '$filter', '$http', '$window', '$location', '$timeout',
	function($scope, $filter, $http, $window, $location, $timeout) {		
		...
		$scope.getDetailForQuotation = function(enquiryid){
			$scope.invoicetypeidadd = 2;
			
			var link = baseUrl+'getEnquiryDetailsById?enquiryid='+enquiryid;    	
			$http.get(link).success(function(data, status, headers, config) {
				$scope.getEnquiryDetailById = data; 
				
				$scope.clientidadd = $scope.getEnquiryDetailById.clientId;
		...
		$scope.generateQuatation = function(enquiryid) {						
