app.controller('taskCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' ,
...
/*	$scope.sendEnquiryNotification = function() {
		
		var link = baseUrl+'sendEnquiryNotification';
		$http.post(link).success(function(data, status, headers,config) {
			$scope.sendnotification = data;
		}).error(function(data, status, headers,config) {
			$scope.sendnotification = "Response Fail";
		});
	}*/
	...
	$scope.fromdate = $scope.todyadate.setDate($scope.todyadate.getDate());
	$scope.fromdate = $filter('date')(new Date($scope.fromdate), "dd/MM/yyyy");
	$scope.todate = $scope.todyadate.setDate($scope.todyadate.getDate() + 30);
...
$scope.addTask = function(){
		...
		$http({url: link, method: "POST", data: $scope.task}).success(function(data, status, headers, config) {
			...
			/*$timeout(function() {						
				$window.location.href = adminurl + 'addTask?taskId='+$scope.productID;
			}, 2000);*/
			
		}).error(function(data, status, headers, config) {
			$scope.taskData = "Response Fail";
	...
	$scope.followupmemberlist = [];
	$scope.addTaskFollowupMemberRow = function() {
		
		alert("entered");
		if($scope.employeeId) {			
			$scope.erroremployeeName = 1;
			$scope.msgemployeeId = "Please select an employee!";
			document.getElementById("employeeId").focus();
		} else {
			var link = baseUrl+'getUserDetailById?userid='+$scope.followupemployeeid;			
			$http.get(link).success(function(data, status, headers, config) {
