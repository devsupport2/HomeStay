app.controller('enquiryCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' ,
	...
	*/
	...
	$scope.editUser = function(clientid) {					
		...
		if($scope.clientcategory == undefined ) {
			$scope.clientcategory = 0;
		}
		///uiuiteamuiteam
		if(!$scope.firstName) {				
			$scope.errorFirstName = 1;
	...
	$scope.addTaskFollowUp = function(){
		
		
		var link = baseUrl+'AddTaskFollowUp';
		$http({url: link, method: "POST", data: $scope.taskFollowUp}).success(function(data, status, headers, config) {
			$scope.TaskFollowsUp = data;
			//toastr.success('Data Saved!');
		//$scope.getAllCityList();
			
		}).error(function(data, status, headers, config) {
			$scope.TaskFollowsUp = "Response Fail";
...
$scope.addTaskEmployeeList = function(){
		
		alert("entered");	
	
	
		var link = baseUrl+'AddTaskEmployee';
		$http({url: link, method: "POST", data: $scope.taskEmployee}).success(function(data, status, headers, config) {
			$scope.TaskEmployee = data;
			//toastr.success('Data Saved!');
		//$scope.getAllCityList();
			
		}).error(function(data, status, headers, config) {
			$scope.TaskEmployee = "Response Fail";
			//toastr.error('Something went wrong!');
		});
